...
export function useRestoreAdminUser() {
  ...
  return useMutation<AiAccessStatusResponse, unknown, string, RestoreAdminUserContext>({
      ...
      if (previousUsers) {
        queryClient.setQueryData<AdminUsersListResponse>(queryKeys.admin.users(), {
          ...
          users: previousUsers.users.map((user) =>
            user.id === userId ? { ...user, aiAccessStatus: AiAccessStatus.APPROVED } : user,
          ),
        });
      }
